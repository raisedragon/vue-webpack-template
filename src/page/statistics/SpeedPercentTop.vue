<template>
<div>
    <TopOptions ref="topOptions" @numChange="getSpeedPercentTop" @methodChange="getSpeedPercentTop"></TopOptions>
    <div ref="chartWrap" class="chart-tab-pane">
        ...
        <div style="width: 50%;">
            <el-table :data="tableData"
                ...
                <el-table-column prop="in" label="接收利用率" align="center" ></el-table-column>

                <el-table-column prop="out" label="发送利用率" align="center" ></el-table-column>

                <el-table-column prop="value" label="总利用率" align="center" ></el-table-column>
...
//                                }], false)
        ...
        methods: {
            getSpeedPercentTop(num, method) {
                const url = '/top/query?id=2&num=' + num + '&method=' + method;
                this.$http.get(url)
                    .then(({data})=>{
                        this.tableData = [];
                        for (var i = 0; i < data.length; i++) {
                            ...
                            this.tableData[i] = {ip:info[0], iface:ifaceStr, in:data[i].rawData["switch.if.InSpeedPercent"] + "%", out:data[i].rawData["switch.if.OutSpeedPercent"] + "%", value:data[i].value + "%", rawValue:data[i].value};
                        }
                        ...
                        this.tableData.forEach(item=>{
                            ...
                            publishOpt.series[0].data.push(item.rawValue);
                        })
        ...
        mounted() {
            this.getSpeedPercentTop(this.$refs['topOptions'].num, this.$refs['topOptions'].method);
        },
